...
int setupDatagramSocket(UsageEnvironment& env, Port port) {
  ...
  if (port.num() != 0 || ReceivingInterfaceAddr != INADDR_ANY) {
    ...
    if (bind(newSocket, (struct sockaddr*)&name, sizeof name) != 0) {
      char tmpBuffer[100];
      sprintf_s(tmpBuffer,100, "bind() error (port number: %d): ",
	      ntohs(port.num()));
...
Boolean writeSocket(UsageEnvironment& env,
  ...
  do {
    ...
    if (bytesSent != (int)bufferSize) {
      char tmpBuf[100];
      sprintf_s(tmpBuf,100, "writeSocket(%d), sendTo() error: wrote %d bytes instead of %u: ", socket, bytesSent, bufferSize);
...
netAddressBits ourIPAddress(UsageEnvironment& env) {
  ...
  if (ourAddress == 0) {
    ...
    if (badAddressForUs(from)) {
      char tmp[100];
      sprintf_s(tmp, 100,"This computer has an invalid IP address: %s", AddressString(from).val());
